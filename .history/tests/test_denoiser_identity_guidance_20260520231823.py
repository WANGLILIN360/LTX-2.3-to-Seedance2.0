...
class TestGuidedDenoiseRuntime:
    ...
    def test_noref_result_fed_to_guider(self, guider_with_id, video_state, noref_video_state, v_context):
        ...
        # identity delta on target positions: 0.5 * (1.0 - 2.0) = -0.5
        # target positions: 1.0 + (-0.5) = 0.5
        # ref positions: 1.0 + 0 = 1.0 (no delta for ref tokens)
        ...
        assert torch.allclose(denoised[..., ref_mask], torch.tensor(1.0), atol=1e-4)
