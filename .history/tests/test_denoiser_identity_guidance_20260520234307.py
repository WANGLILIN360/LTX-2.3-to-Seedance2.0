...
class TestGuidedDenoiseRuntime:
    ...
    def test_noref_result_fed_to_guider(self, guider_with_id, video_state, noref_video_state, v_context):
        ...
        print(f"denoised shape: {denoised.shape}, target_mask shape: {target_mask.shape}, sum: {target_mask.sum()}")
        print(f"target unique: {denoised[:, target_mask, :].unique()}")
        print(f"ref unique: {denoised[:, ~target_mask, :].unique()}")
