...
@pytest.fixture()
def video_state():
    """Video state with target tokens (denoise=1) and reference tokens (denoise=0)."""
    from ltx_core.types import LatentState
    target_len = 12
    ref_len = 8
    total_len = target_len + ref_len
    return LatentState(
        latent=torch.randn(1, 128, total_len),
        denoise_mask=torch.cat([
            torch.ones(1, 1, target_len),   # target: denoise
            torch.zeros(1, 1, ref_len),       # ref: frozen
        ], dim=2),
        positions=torch.randn(1, total_len, 3),
        clean_latent=torch.zeros(1, 128, total_len),
...
def noref_video_state():
    """Noref state: only target tokens (no reference tokens, all denoise=1)."""
    from ltx_core.types import LatentState
    return LatentState(
...
class TestGuidedDenoiseRuntime:
    ...
    def test_noref_not_called_when_disabled(self, guider_no_id, video_state, v_context):
        ...
        class MockTransformer:
            def __call__(self, video=None, audio=None, perturbations=None):
                ...
                return torch.randn(B, 128, 20), None
    ...
    def test_noref_result_fed_to_guider(self, guider_with_id, video_state, noref_video_state, v_context):
        """Verify noref transformer output is actually used in guider.calculate."""
        from ltx_pipelines.utils.denoisers import _guided_denoise
        ...
        class MockTransformer:
            def __call__(self, video=None, audio=None, perturbations=None):
                ...
                B = video.latent.shape[0] if video is not None else 1
                seq_len = video.latent.shape[2] if video is not None else 20
                # Return different values for each call so we can verify
                return torch.full((B, 128, seq_len), float(call_idx[0])), None
        ...
        # With cfg=1, id=0.5: result = cond + 0.5*(cond - noref)
        # cond = 1.0 (first call), noref = 2.0 (second call)
        # result = 1.0 + 0.5*(1.0 - 2.0) = 1.0 - 0.5 = 0.5
        assert result_v is not None
        assert torch.allclose(result_v.denoised, torch.tensor(0.5), atol=1e-4)
...
class TestPipelineNorefStateRuntime:
    ...
    def test_noref_state_has_no_ref_tokens(self, video_state, noref_video_state):
        ...
        assert noref_video_state.latent.shape[2] < video_state.latent.shape[2]
    ...
    def test_noref_state_same_noise_pattern(self):
        ...
        """
        from ltx_core.types import LatentState
        # Simulated: both have same target token count in the base latent
        # but cond has additional ref tokens appended
        target_tokens = 80
        ref_tokens = 20
        cond_state = LatentState(
            latent=torch.randn(1, 128, target_tokens + ref_tokens),
            denoise_mask=torch.cat([
                torch.ones(1, 1, target_tokens),  # target: denoise
                torch.zeros(1, 1, ref_tokens),     # ref: no denoise
            ], dim=2),
            positions=torch.randn(1, target_tokens + ref_tokens, 3),
            clean_latent=torch.zeros(1, 128, target_tokens + ref_tokens),
            attention_mask=torch.ones(1, 1, 1, target_tokens + ref_tokens),
        )
        noref_state = LatentState(
            latent=torch.randn(1, 128, target_tokens),  # no ref tokens
            denoise_mask=torch.ones(1, 1, target_tokens),
            positions=torch.randn(1, target_tokens, 3),
            clean_latent=torch.zeros(1, 128, target_tokens),
            attention_mask=torch.ones(1, 1, 1, target_tokens),
        )
        # Key invariant: noref has fewer tokens
        assert noref_state.latent.shape[2] < cond_state.latent.shape[2]
