...
class TestGuidedDenoiseNoref:
    ...
    Note: ltx_pipelines has a missing 'multigpu' dependency, so we test
    the core logic by importing _guided_denoise directly from the module file.
    """

    @pytest.fixture(autouse=True)
    def _import_denoiser(self):
        """Import _guided_denoise directly, bypassing the broken top-level import."""
        import importlib.util

        # Add ltx_pipelines to path so relative imports work
        pipelines_src = str(Path(__file__).resolve().parent.parent / "packages" / "ltx-pipelines" / "src")
        if pipelines_src not in sys.path:
            sys.path.insert(0, pipelines_src)

        # Stub the missing multigpu module with required attributes
        for mod_name, attrs in [
            ("ltx_pipelines.multigpu", {}),
            ("ltx_pipelines.multigpu.delegating_builder", {"DelegatingBuilder": type("DelegatingBuilder", (), {})}),
        ]:
        ...
        spec = importlib.util.spec_from_file_location(
            ...
            str(Path(__file__).resolve().parent.parent / "packages" / "ltx-pipelines" / "src" / "ltx_pipelines" / "utils" / "denoisers.py"),
            submodule_search_locations=[],
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        self._guided_denoise = mod._guided_denoise
...
class TestDenoiserNorefPassthrough:
    """Test that denoisers accept and store noref state.

    Since ltx_pipelines has a missing 'multigpu' dependency, we test
    by importing the denoiser classes directly from the module file.
    """

    @pytest.fixture(autouse=True)
    def _import_denoisers(self):
        import importlib.util
        ...
        spec = importlib.util.spec_from_file_location(
            "ltx_pipelines.utils.denoisers2",
            str(Path(__file__).resolve().parent.parent / "packages" / "ltx-pipelines" / "src" / "ltx_pipelines" / "utils" / "denoisers.py"),
            submodule_search_locations=[],
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        self.GuidedDenoiser = mod.GuidedDenoiser
        self.FactoryGuidedDenoiser = mod.FactoryGuidedDenoiser
    ...
    def test_factory_denoiser_stores_noref(self):
        ...
        denoiser = self.FactoryGuidedDenoiser(
            v_context=torch.randn(1, 20, 4096),
            a_context=None,
