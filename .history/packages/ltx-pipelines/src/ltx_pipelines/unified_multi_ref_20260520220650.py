...
class UnifiedMultiRefPipeline:
    ...
    def _build_multi_ref_conditionings(
        ...
        # Lazy-initialize slot embedding based on latent channel dimension
        if self._slot_embedding is None:
            # Infer embed_dim from the first reference latent's channel dim
            # The latent channels match the transformer's inner_dim
            embed_dim = ref_items[0].latent.shape[1]
