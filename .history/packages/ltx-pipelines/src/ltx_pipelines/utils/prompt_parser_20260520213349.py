...
def _extract_attributes_for_mention(
    ...
    # Also look at a window before the mention for "for X" patterns
    mention_start = mention_match.start()
    lookback_start = max(0, mention_start - 80)
    ...
    # Check each attribute keyword in the context with word boundary matching
    for keyword, attr in _ATTRIBUTE_KEYWORDS.items():
        # Use word boundary regex to avoid false positives
        # e.g., "facial" should not match "superficial", "pan" should not match "panorama"
        if re.search(r'\b' + re.escape(keyword) + r'\b', context) and attr not in seen:
