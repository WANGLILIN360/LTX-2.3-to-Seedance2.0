...
def _guided_denoise(  # noqa: PLR0913,PLR0915
    ...
    *,
    ...
    # (where timesteps > 0, i.e., denoise_mask=1 in patchified space) so the
    # shapes match for the guidance delta. The delta is then broadcast back to
    # the full cond_v shape (ref positions get 0 delta).
    ...
    # Compute identity guidance delta separately (cannot pass to guider.calculate
    # because cond_v and noref_v have different shapes).
    # Target tokens have timesteps > 0 in the batched_video Modality;
    # ref tokens have timesteps = 0.
    ...
    if v_needs_noref and isinstance(noref_v, torch.Tensor) and isinstance(cond_v, torch.Tensor):
        # Identify target tokens in patchified space: timesteps > 0 means denoise_mask=1
        # batched_video.timesteps shape: (B, T) where T is the sequence length
        target_mask = batched_video.timesteps[:1] > 0  # (1, 1, T) — use first batch item
        target_mask_1d = target_mask.squeeze(0).squeeze(0)  # (T,)
        id_cond_v = cond_v[:, target_mask_1d, :]  # (1, T_target, D) — target-only
        ...
        full_delta_v[:, target_mask_1d, :] = id_delta_v
    ...
    if a_needs_noref and isinstance(noref_a, torch.Tensor) and isinstance(cond_a, torch.Tensor):
        target_mask = batched_audio.timesteps[:1] > 0
        target_mask_1d = target_mask.squeeze(0).squeeze(0)
        id_cond_a = cond_a[:, target_mask_1d, :]
        id_delta_a = audio_guider.params.identity_guidance_scale * (id_cond_a - noref_a)
        full_delta_a = torch.zeros_like(cond_a)
        full_delta_a[:, target_mask_1d, :] = id_delta_a
        noref_a = 0.0
