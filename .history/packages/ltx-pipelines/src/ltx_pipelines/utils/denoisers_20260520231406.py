...
def _guided_denoise(  # noqa: PLR0913,PLR0915
    ...
    *,
    ...
    # and feed the result into guider.calculate() as uncond_noref.
    ...
    #
    # IMPORTANT: cond_v includes both target and reference tokens, but noref_v
    # only has target tokens. We extract the target-only portion from cond_v
    # (where denoise_mask=1) so the shapes match for the guidance delta.
    ...
    # For identity guidance, extract target-only from cond_v to match noref_v shape.
    # Reference tokens have denoise_mask=0; target tokens have denoise_mask=1.
    id_cond_v = cond_v
    if v_needs_noref and isinstance(noref_v, torch.Tensor) and isinstance(cond_v, torch.Tensor):
        target_mask = video_state.denoise_mask.squeeze(0).squeeze(0).bool()  # (seq_len,)
        id_cond_v = cond_v[..., target_mask]  # target-only portion
    id_cond_a = cond_a
    if a_needs_noref and isinstance(noref_a, torch.Tensor) and isinstance(cond_a, torch.Tensor):
        target_mask = audio_state.denoise_mask.squeeze(0).squeeze(0).bool()  # (seq_len,)
        id_cond_a = cond_a[..., target_mask]  # target-only portion

    # Calculate identity guidance delta on target-only tokens, then merge back.
    # The delta is applied only to target positions; ref positions keep cond prediction.
    if v_needs_noref and isinstance(noref_v, torch.Tensor):
        id_delta_v = video_guider.params.identity_guidance_scale * (id_cond_v - noref_v)
        # Broadcast delta back to full cond_v shape (ref positions get 0 delta)
        full_delta_v = torch.zeros_like(cond_v)
        target_mask = video_state.denoise_mask.squeeze(0).squeeze(0).bool()
        full_delta_v[..., target_mask] = id_delta_v
        # Apply delta on top of the standard guidance (without identity term)
        noref_v_for_calc = cond_v - full_delta_v / max(video_guider.params.identity_guidance_scale, 1e-8)
        # Simpler: just pass noref_v and let calculate() handle it, but we need
        # cond_v and noref_v to have same shape. Use the target-only trick:
        # We compute the identity delta separately and add it.
        noref_v = 0.0  # Don't pass to calculate — we handle it manually
        id_guidance_delta_v = full_delta_v
    else:
        id_guidance_delta_v = 0.0

    if a_needs_noref and isinstance(noref_a, torch.Tensor):
        id_delta_a = audio_guider.params.identity_guidance_scale * (id_cond_a - noref_a)
        full_delta_a = torch.zeros_like(cond_a)
        target_mask = audio_state.denoise_mask.squeeze(0).squeeze(0).bool()
        full_delta_a[..., target_mask] = id_delta_a
        noref_a = 0.0
        id_guidance_delta_a = full_delta_a
    else:
        id_guidance_delta_a = 0.0
