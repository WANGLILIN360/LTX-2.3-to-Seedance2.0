...
def _guided_denoise(  # noqa: PLR0913,PLR0915
    ...
    *,
    ...
    # NOTE: noref state has different sequence length (no reference tokens), so it CANNOT
    # be batched with the other passes. It runs as a separate transformer forward call.
    ...
    # Identity guidance: separate forward pass with no-reference state.
    # The noref state has a different sequence length (no reference tokens appended),
    # so it cannot be batched with the other passes. We run a second transformer call
    # and compute the guidance delta manually.
    # Uses the same positive text context as the cond pass — identity guidance
    # extrapolates between "text+references" and "text only (no references)".
    #
    # Since noref_v has fewer tokens (no ref tokens), we zero-pad it to match
    # cond_v's shape. The delta cond - noref_padded will be non-zero only at
    # target positions; ref positions get 0 delta (since noref_padded is 0 there
    # and cond's ref positions aren't meaningful for guidance anyway).
    noref_v: torch.Tensor | float = 0.0
    noref_a: torch.Tensor | float = 0.0
    if v_needs_noref or a_needs_noref:
        # Get the original positive context from the cond pass
        _, cond_v_ctx, cond_a_ctx, _ = passes[0]
        noref_batched_video = None
        if noref_video_state is not None:
            noref_batched_video = modality_from_latent_state(
                noref_video_state,
                cond_v_ctx,
                sigma.expand(noref_video_state.latent.shape[0]),
                enabled=not v_skip,
            )
        noref_batched_audio = None
        if noref_audio_state is not None:
            noref_batched_audio = modality_from_latent_state(
                noref_audio_state,
                cond_a_ctx,
                sigma.expand(noref_audio_state.latent.shape[0]),
                enabled=not a_skip,
            )
        noref_all_v, noref_all_a = transformer(
            video=noref_batched_video, audio=noref_batched_audio,
            perturbations=BatchedPerturbationConfig([PerturbationConfig.empty()]),
        )
        noref_v = noref_all_v if noref_all_v is not None else 0.0
        noref_a = noref_all_a if noref_all_a is not None else 0.0

    # Compute identity guidance delta by zero-padding noref to match cond shape.
    # This works because: at target positions, noref has prediction → delta = scale*(cond-noref).
    # At ref positions, noref is 0 → delta = scale*cond, but ref positions have
    # denoise_mask=0 so they're not used in the final denoised output anyway.
    #
    # We compute the delta manually and pass noref_v=cond_v to guider.calculate
    # so the built-in identity_guidance_scale term becomes scale*(cond-cond)=0,
    # avoiding double-counting.
    id_guidance_delta_v: torch.Tensor | float = 0.0
    if v_needs_noref and isinstance(noref_v, torch.Tensor) and isinstance(cond_v, torch.Tensor):
        noref_padded_v = torch.zeros_like(cond_v)
        # Noref tokens correspond to the first T_noref positions (target tokens)
        T_noref = noref_v.shape[1]
        noref_padded_v[:, :T_noref, :] = noref_v
        id_guidance_delta_v = video_guider.params.identity_guidance_scale * (cond_v - noref_padded_v)
        noref_v = cond_v  # Neutralize guider.calculate's identity term: scale*(cond-cond)=0
        import sys; print(f"DEBUG id_guidance: cond_v shape={cond_v.shape}, noref_v shape={noref_v.shape if isinstance(noref_v, torch.Tensor) else '0.0'}, delta shape={id_guidance_delta_v.shape}, delta target unique={id_guidance_delta_v[:, :T_noref, :].unique()}, delta ref unique={id_guidance_delta_v[:, T_noref:, :].unique()}", file=sys.stderr)

    id_guidance_delta_a: torch.Tensor | float = 0.0
    if a_needs_noref and isinstance(noref_a, torch.Tensor) and isinstance(cond_a, torch.Tensor):
        noref_padded_a = torch.zeros_like(cond_a)
        T_noref = noref_a.shape[1]
        noref_padded_a[:, :T_noref, :] = noref_a
        id_guidance_delta_a = audio_guider.params.identity_guidance_scale * (cond_a - noref_padded_a)
        noref_a = cond_a  # Neutralize guider.calculate's identity term

    denoised_video = last_denoised_video if v_skip else video_guider.calculate(cond_v, uncond_v, ptb_v, mod_v, noref_v) + id_guidance_delta_v
    denoised_audio = last_denoised_audio if a_skip else audio_guider.calculate(cond_a, uncond_a, ptb_a, mod_a, noref_a) + id_guidance_delta_a
    import sys; print(f"DEBUG final: denoised_video shape={denoised_video.shape if isinstance(denoised_video, torch.Tensor) else type(denoised_video)}, unique={denoised_video.unique() if isinstance(denoised_video, torch.Tensor) else 'N/A'}, v_skip={v_skip}", file=sys.stderr)
    return (
