...
class UnifiedMultiRefPipeline:
    ...
    def _build_multi_ref_conditionings(
        ...
        # Lazy-initialize slot embedding based on patchified token dimension
        if self._slot_embedding is None:
            # Patchified token dim = in_channels * prod(patch_size).
            # For default LTX-2.3: in_channels=128, patch_size=(1,1,1) → dim=128.
            # This matches the dimension of tokens after patchify() but before
            # the transformer's input projection.
            embed_dim = ref_items[0].latent.shape[1]  # C dimension
            self._slot_embedding = ReferenceSlotEmbedding(
                embed_dim=embed_dim,
            ).to(device=self.device, dtype=self.dtype)
            logger.info(f"Initialized ReferenceSlotEmbedding with embed_dim={embed_dim}")

        return [
            UnifiedMultiReferenceConditioning(
                references=ref_items,
                slot_embedding=self._slot_embedding,
                attribute_weights=attribute_weights,
            )
        ]
