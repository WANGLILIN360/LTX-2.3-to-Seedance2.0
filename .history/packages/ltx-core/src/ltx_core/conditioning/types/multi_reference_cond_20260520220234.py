...
class UnifiedMultiReferenceConditioning(ConditioningItem):
    ...
    and injects them into the latent state with explicit slot binding.

    Architecture (Seedance 2.0 Binding Logic):
    - Each reference gets a slot_id (e.g., "Image1") that binds it to the
      corresponding @mention in the text prompt.
    - A :class:`ReferenceSlotEmbedding` provides learnable binding vectors
      that are added to both the reference latent tokens AND the text
      context at the @mention positions. This creates a shared signal that
      the model's cross-attention can use to associate each reference with
      its @mention.
    - Different reference groups are isolated (don't attend to each other)
      to prevent attribute confusion.
    - The model learns semantic routing (e.g., "for identity" → focus on
      face features) from the text context via cross-attention, NOT from
      hand-crafted mask weights.
    ...
    The binding chain:
        Prompt: "Man @Image1 for identity walks in @Image2 park. @Video1 camera."
                                    ↓                           ↓            ↓
        Text context: [...slot_emb_Image1...] [...slot_emb_Image2...] [...slot_emb_Video1...]
                                    ↕                           ↕            ↕
        Latent seq:  [target] [ref_tokens+slot_emb_Image1] [ref+slot_emb_Image2] [ref+slot_emb_Video1]

    Args:
        references: List of :class:`ReferenceItem` to inject.
        slot_embedding: Optional :class:`ReferenceSlotEmbedding` for binding.
            If None, slot binding is disabled (backward compatible).
    ...
    def __init__(
        ...
        slot_embedding: ReferenceSlotEmbedding | None = None,
