...
with explicit attribute tags controlling what the model should extract
(identity, motion, style, camera, audio_rhythm, etc.).

The conditioning appends reference tokens to the latent sequence with
attribute-aware attention masks that control cross-attention strength
between each reference group and the noisy target tokens.
"""
...
class ReferenceItem:
    ...
    Attributes:
        ...
        attribute_tags: Which semantic attributes this reference should
            contribute. Controls the attention mask weight.
...
class UnifiedMultiReferenceConditioning(ConditioningItem):
    ...
    and injects them into the latent state with attribute-aware attention masks.

    Each reference's tokens are appended to the sequence. The attention mask
    is constructed so that:
    - Different reference groups do **not** attend to each other (isolated)
    - Each reference group attends to noisy target tokens with a strength
      determined by its ``attribute_tags`` (attribute routing)
    - Noisy target tokens attend to each reference group with the same weight

    This design mirrors the block mask structure in ``mask_utils.py`` but
    extends it to support N independent reference groups with per-group
    attention weights.
    ...
    def _apply_video_reference(
        ...
        # Compute attribute-aware attention weight
        attn_weight = self._compute_attention_weight(ref)

        # Build attention mask with attribute routing
        new_attention_mask = update_attention_mask(
            latent_state=latent_state,
            attention_mask=attn_weight,
    ...
    def _apply_image_at_frame(
        ...
        state.latent[:, start_token:stop_token] = tokens
        state.clean_latent[:, start_token:stop_token] = tokens
        state.denoise_mask[:, start_token:stop_token] = 1.0 - ref.strength
