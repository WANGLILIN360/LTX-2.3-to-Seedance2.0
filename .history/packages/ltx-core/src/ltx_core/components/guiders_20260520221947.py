...
class MultiModalGuiderParams:
    ...
    "Identity guidance scale (ID-LoRA style). Extrapolates between conditioned-with-reference "
    "and conditioned-without-reference predictions to amplify identity-specific features. "
    "0.0 = disabled, >0 = stronger identity preservation from references."
    skip_step: int = 0
...
class MultiModalGuider:
    ...
    def calculate(
        ...
        uncond_noref: torch.Tensor | float = 0.0,
    ) -> torch.Tensor:
        """
        ...
        Identity guidance (ID-LoRA style): extrapolates between cond (with references) and
        uncond_noref (without references but with text) to amplify identity-specific features.
        """
        pred = (
            cond
            ...
            + self.params.identity_guidance_scale * (cond - uncond_noref)
        )
    ...
    def do_identity_guidance(self) -> bool:
        """Returns True if the guider is doing identity guidance (ID-LoRA style)."""
        return not math.isclose(self.params.identity_guidance_scale, 0.0)
