...
class ValidationSampler:
    ...
    def _generate_with_multi_reference(
        ...
        Concatenates multiple image/video/audio reference latents with the target
        latent, using isolated attention between reference groups. Supports
        identity guidance via a separate no-reference forward pass.
        """
        ...
        # Apply multi-reference conditioning to target state
        if ref_items:
            multi_ref_cond = UnifiedMultiReferenceConditioning(references=ref_items)
            combined_clean_state = multi_ref_cond.apply_to(target_clean_state, video_tools)
        else:
            combined_clean_state = target_clean_state

        # Build no-reference state for identity guidance
        noref_video_state = None
        if config.identity_guidance_scale > 0:
            noref_video_state = target_clean_state
        ...
        for item in ref_items:
            total_ref_len += item.latent.shape[1]
