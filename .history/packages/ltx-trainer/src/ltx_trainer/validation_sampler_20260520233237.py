...
class GenerationConfig:
    ...
    reference_images: list[Tensor] | None = None  # For multi-ref: list of [C, H, W] in [0, 1]
    reference_videos: list[Tensor] | None = None  # For multi-ref: list of [F, C, H, W] in [0, 1]
    reference_audios: list[Tensor] | None = None  # For multi-ref: list of audio waveforms
    identity_guidance_scale: float = 0.0  # Identity guidance scale (0.0 = disabled)
...
class ValidationSampler:
    ...
    def generate(
        ...
        has_multi_ref = config.reference_images or config.reference_videos or config.reference_audios
        if has_multi_ref:
            return self._generate_with_multi_reference(config, device)
    ...
    def _generate_with_multi_reference(
        self, config: GenerationConfig, device: torch.device
    ) -> tuple[Tensor, Tensor | None]:
        """Generate with multiple references (multi-reference conditioning).

        Concatenates multiple image/video reference latents with the target
        latent using the same IC-LoRA concatenation pattern as
        _generate_with_reference, but supporting multiple reference groups.
        Identity guidance is not yet supported in validation (deferred).
        """
        ...
        # Collect all reference latents, positions, and denoise masks
        all_ref_latents = []
        all_ref_positions = []
        all_ref_denoise_masks = []

        # Process image references
        if config.reference_images:
            for i, ref_image in enumerate(config.reference_images):
                # ref_image is [C, H, W] in [0, 1]
                encoded = self._encode_conditioning_image(
                    ref_image, config.height, config.width, device
                )
                patchified = self._video_patchifier.patchify(encoded)
                ref_seq_len = patchified.shape[1]
                ref_positions = self._get_single_frame_positions(
                    config.height, config.width, config.frame_rate, device, torch.bfloat16
                )
                ref_denoise_mask = torch.zeros(1, ref_seq_len, 1, device=device, dtype=torch.float32)

                all_ref_latents.append(patchified)
                all_ref_positions.append(ref_positions)
                all_ref_denoise_masks.append(ref_denoise_mask)

        # Process video references
        if config.reference_videos:
            for i, ref_vid in enumerate(config.reference_videos):
                # ref_vid is [F, C, H, W] in [0, 1]
                ref_preprocessed = self._preprocess_multi_ref_video(ref_vid, config)
                ref_latent, ref_positions = self._encode_video(ref_preprocessed, config.frame_rate, device)
                if config.reference_downscale_factor != 1:
                    ref_positions = ref_positions.clone()
                    ref_positions[:, 1, ...] *= config.reference_downscale_factor
                    ref_positions[:, 2, ...] *= config.reference_downscale_factor
                ref_seq_len = ref_latent.shape[1]
                ref_denoise_mask = torch.zeros(1, ref_seq_len, 1, device=device, dtype=torch.float32)
        ...
        # Build combined state (references + target)
        if all_ref_latents:
            combined_ref_latent = torch.cat(all_ref_latents, dim=1)
            combined_ref_positions = torch.cat(all_ref_positions, dim=2)
            combined_ref_denoise_mask = torch.cat(all_ref_denoise_masks, dim=1)
            total_ref_len = combined_ref_latent.shape[1]

            combined_clean_state = LatentState(
                latent=torch.cat([combined_ref_latent, target_clean_state.latent], dim=1),
                denoise_mask=torch.cat([combined_ref_denoise_mask, target_clean_state.denoise_mask], dim=1),
                positions=torch.cat([combined_ref_positions, target_clean_state.positions], dim=2),
                clean_latent=torch.cat([combined_ref_latent, target_clean_state.clean_latent], dim=1),
            )
        else:
            combined_clean_state = target_clean_state
            total_ref_len = 0
        ...
        # Extract target portion and decode
        if total_ref_len > 0:
            target_latent = combined_state.latent[:, total_ref_len:]
        else:
            target_latent = combined_state.latent
    ...
    @staticmethod
    def _preprocess_multi_ref_video(ref_video: Tensor, config: GenerationConfig) -> Tensor:
        """Preprocess a multi-reference video: resize, crop, convert to model format.
        Args:
            ref_video: Reference video [F, C, H, W] in [0, 1]
            config: Generation configuration
        Returns:
        ...
        # Convert to [B, C, F, H, W] and trim to valid frame count
    ...
    def _get_single_frame_positions(
        self, height: int, width: int, fps: float, device: torch.device, dtype: torch.dtype
    ) -> Tensor:
        """Get positions for a single-frame image reference."""
        ...
        latent_coords = self._video_patchifier.get_patch_grid_bounds(
            output_shape=VideoLatentShape(
                frames=1, height=latent_height, width=latent_width, batch=1, channels=128
            ),
            device=device,
        )
