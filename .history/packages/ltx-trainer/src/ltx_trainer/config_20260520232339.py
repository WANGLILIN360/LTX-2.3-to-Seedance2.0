...
from ltx_trainer.training_strategies.multi_reference import MultiReferenceConfig
...
TrainingStrategyConfig = Annotated[
    Annotated[TextToVideoConfig, Tag("text_to_video")] | Annotated[VideoToVideoConfig, Tag("video_to_video")] | Annotated[MultiReferenceConfig, Tag("multi_reference")],
...
class ValidationConfig(ConfigBaseModel):
    ...
    reference_images: list[list[str]] | None = Field(
        default=None,
        description="List of lists of image paths for multi-reference validation. "
        "Outer list has one entry per prompt, inner list has per-reference image paths. "
        "Example: [[\"img1_a.png\", \"img1_b.png\"], [\"img2_a.png\"]] for 2 prompts.",
    )

    reference_video_paths: list[list[str]] | None = Field(
        default=None,
        description="List of lists of video paths for multi-reference validation. "
        "Outer list has one entry per prompt, inner list has per-reference video paths.",
    )

    identity_guidance_scale: float = Field(
        default=0.0,
        description="Identity guidance scale for multi-reference validation. "
...
class LtxTrainerConfig(ConfigBaseModel):
    ...
    def validate_strategy_compatibility(self) -> "LtxTrainerConfig":
        ...
        # Check that LoRA config is provided when using multi_reference strategy
        if self.training_strategy.name == "multi_reference" and self.model.training_mode != "lora":
            raise ValueError("Training mode must be 'lora' when using multi_reference strategy")
