...
class LtxvTrainer:
    ...
    def _training_step(self, batch: dict[str, dict[str, Tensor]]) -> TrainingStepOutput:
        ...
        # Run no-reference forward pass for identity guidance if needed
        noref_video_pred = None
        if model_inputs.noref_video is not None:
            noref_video_pred, _ = self._transformer(
                video=model_inputs.noref_video,
                audio=None,  # No audio in noref pass
                perturbations=None,
            )

        # Use strategy to compute loss
        loss = self._training_strategy.compute_loss(video_pred, audio_pred, model_inputs, noref_video_pred)
    ...
    def _collect_trainable_params(self) -> None:
        ...
        # Collect extra trainable params from training strategy (e.g., slot embeddings)
        if hasattr(self._training_strategy, 'get_extra_trainable_params'):
            extra_params = self._training_strategy.get_extra_trainable_params()
            self._trainable_params.extend(extra_params)
            if extra_params:
                logger.debug(f"Strategy extra trainable params: {sum(p.numel() for p in extra_params):,}")
    ...
    def _sample_videos(self, progress: TrainingProgress) -> list[tuple[int, Path]]:
        ...
        use_multi_ref_images = self._config.validation.reference_images is not None
        use_multi_ref_videos = self._config.validation.reference_video_paths is not None
        ...
        for local_i, (prompt_idx, save_output) in enumerate(work):
            ...
            # Load multi-reference images/videos if provided
            multi_ref_images = None
            if use_multi_ref_images:
                image_paths = self._config.validation.reference_images[prompt_idx]
                multi_ref_images = []
                for img_path in image_paths:
                    img = open_image_as_srgb(img_path)
                    multi_ref_images.append(F.to_tensor(img))  # [C, H, W] in [0, 1]

            multi_ref_videos = None
            if use_multi_ref_videos:
                video_paths = self._config.validation.reference_video_paths[prompt_idx]
                multi_ref_videos = []
                for vid_path in video_paths:
                    vid, _ = read_video(vid_path, max_frames=num_frames)  # [F, C, H, W] in [0, 1]
                    multi_ref_videos.append(vid)
            ...
            gen_config = GenerationConfig(
                ...
                reference_images=multi_ref_images,
                reference_videos=multi_ref_videos,
                identity_guidance_scale=self._config.validation.identity_guidance_scale,
