...
class MultiReferenceStrategy(TrainingStrategy):
    ...
    def _process_image_references(
        ...
        # Image references may come as a list or single tensor
        ref_latents = ref_data["latents"]
        if ref_latents.dim() == 5:
            # Single image reference: [B, C, 1, H, W]
            tokens = self._video_patchifier.patchify(ref_latents)
            ...
            # Scale positions if downscale factor > 1
    ...
    def _process_video_references(
        ...
        if ref_latents.dim() == 5:
            ...
            # Infer and cache downscale factor
...
class _ReferenceGroup:
    ...
    __slots__ = ("tokens", "positions", "seq_len", "modality", "attributes")
