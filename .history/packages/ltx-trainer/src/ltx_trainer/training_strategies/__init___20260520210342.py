...
- Multi-reference training (unified multi-modal conditioning with attribute routing)
...
from ltx_trainer.training_strategies.multi_reference import (
    MultiReferenceConfig,
    MultiReferenceStrategy,
)
...
TrainingStrategyConfig = TextToVideoConfig | VideoToVideoConfig | MultiReferenceConfig
...
def get_training_strategy(config: TrainingStrategyConfig) -> TrainingStrategy:
    ...
    match config:
        ...
        case MultiReferenceConfig():
            strategy = MultiReferenceStrategy(config)
        case _:
