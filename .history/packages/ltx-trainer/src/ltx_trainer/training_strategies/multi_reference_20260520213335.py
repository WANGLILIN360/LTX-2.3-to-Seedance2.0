...
class MultiReferenceStrategy(TrainingStrategy):
            ...
            for i, img_latent in enumerate(ref_latents):
                if img_latent.dim() == 5:
                    group = self._make_image_ref_group(
                        img_latent, ref_data, batch_size, device, dtype, fps,
                        ref_index=i,
                    )
            ...
            for i, vid_latent in enumerate(ref_latents):
                if vid_latent.dim() == 5:
                    group = self._make_video_ref_group(
                        vid_latent, ref_data, batch_size, device, dtype, fps,
                        ref_index=i,
                    )
                    if group is not None:
                        groups.append(group)
    ...
    def _build_attribute_aware_attention_mask(
        ...
        """Build a 2-D self-attention mask with per-group attribute-aware weights.
        ...
        Where w_i is the attribute-aware attention weight for group i.
        Different reference groups do NOT attend to each other (0).
        """
        if not ref_groups:
            return None
        ...
        for group in ref_groups:
            ...
            w = group.attention_weight

            # Target tokens attend to this reference group with weight w
            mask[:, :target_seq_len, grp_start:grp_end] = w
            # This reference group attends to target tokens with weight w
            mask[:, grp_start:grp_end, :target_seq_len] = w
    ...
    def _apply_reference_dropout(
        ...
        """Apply random dropout to reference groups for robustness."""
        if not groups:
            return groups
        ...
        for group in groups:
            if torch.rand(1).item() >= self.config.reference_dropout_p:
                ...
                logger.debug(f"Dropping {group.modality.value} reference group (dropout)")
...
class _ReferenceGroup:
    ...
    __slots__ = ("tokens", "positions", "seq_len", "modality", "attributes", "attention_weight")
