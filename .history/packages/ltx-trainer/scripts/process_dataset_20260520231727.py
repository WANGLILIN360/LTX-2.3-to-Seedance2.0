...
def preprocess_dataset(  # noqa: PLR0913
    ...
    ref_image_columns: list[str] | None = None,
    ref_video_columns: list[str] | None = None,
    ref_audio_columns: list[str] | None = None,
) -> None:
    ...
    # Process multi-reference columns (image, video, audio references)
    _process_multi_references(
        dataset_file=dataset_file,
        video_column=video_column,
    ...
    if ref_image_columns:
        logger.info(f"Image references processed and saved to ref_image_latents/ directory ({len(ref_image_columns)} references per sample)")
    if ref_video_columns:
        logger.info(f"Video references processed and saved to ref_video_latents/ directory ({len(ref_video_columns)} references per sample)")
    if ref_audio_columns:
        logger.info(f"Audio references processed and saved to ref_audio_latents/ directory ({len(ref_audio_columns)} references per sample)")
...
def _process_multi_references(  # noqa: PLR0913
    dataset_file: str,
    video_column: str,
    output_base: Path,
    model_path: str,
    ...
    """Process multi-reference columns into indexed subdirectories.

    For each reference column, encodes the reference media and saves to:
        ref_image_latents/0/, ref_image_latents/1/, ...  (for image refs)
        ref_video_latents/0/, ref_video_latents/1/, ...  (for video refs)
        ref_audio_latents/0/, ref_audio_latents/1/, ...  (for audio refs)
    """
    has_multi_ref = ref_image_columns or ref_video_columns or ref_audio_columns
    if not has_multi_ref:
        return

    logger.info("Processing multi-reference columns for multi-reference training...")

    # Process image reference columns
    if ref_image_columns:
        for idx, col in enumerate(ref_image_columns):
            ref_dir = output_base / "ref_image_latents" / str(idx)
            logger.info(f"  Processing image reference column '{col}' -> ref_image_latents/{idx}/")

            # Image references use 1-frame buckets (single image per reference)
            image_buckets = [(1, h, w) for f, h, w in resolution_buckets]
            ...
            with free_gpu_memory_context():
                compute_latents(
                    ...
                    main_media_column=video_column,
                    video_column=col,
                    resolution_buckets=image_buckets,
    ...
    # Process video reference columns
    if ref_video_columns:
        reference_buckets = compute_scaled_resolution_buckets(resolution_buckets, reference_downscale_factor)

        for idx, col in enumerate(ref_video_columns):
            ref_dir = output_base / "ref_video_latents" / str(idx)
            logger.info(f"  Processing video reference column '{col}' -> ref_video_latents/{idx}/")
    ...
    # Process audio reference columns
    if ref_audio_columns:
        for idx, col in enumerate(ref_audio_columns):
            ref_dir = output_base / "ref_audio_latents" / str(idx)
            logger.info(f"  Processing audio reference column '{col}' -> ref_audio_latents/{idx}/")
            ...
            with free_gpu_memory_context():
                compute_latents(
                    ...
                    vae_tiling=vae_tiling,
                    with_audio=with_audio,
                    audio_output_dir=str(ref_dir),  # Audio refs saved alongside video latents
                    overwrite=overwrite,
                )
...
def main(  # noqa: PLR0913
    ...
    ref_image_columns: list[str] | None = typer.Option(
        default=None,
        help="Column name(s) for image reference paths (multi-reference training). "
        "Each column is processed into ref_image_latents/<index>/. "
        "Specify multiple times for multiple image references: --ref-image-col ref_img1 --ref-image-col ref_img2",
    ),
    ref_video_columns: list[str] | None = typer.Option(
        default=None,
        help="Column name(s) for video reference paths (multi-reference training). "
        "Each column is processed into ref_video_latents/<index>/. "
        "Specify multiple times for multiple video references.",
    ),
    ref_audio_columns: list[str] | None = typer.Option(
        default=None,
        help="Column name(s) for audio reference paths (multi-reference training). "
        "Each column is processed into ref_audio_latents/<index>/. "
        "Specify multiple times for multiple audio references.",
    ),
) -> None:
    ...
    Examples:
        ...
        python scripts/process_dataset.py dataset.json --resolution-buckets 768x512x97 \\
            --model-path /path/to/ltx2.safetensors --text-encoder-path /path/to/gemma \\
            --with-audio
        # Process dataset with multi-reference columns for multi-reference training
        python scripts/process_dataset.py dataset.json --resolution-buckets 768x768x25 \\
            --model-path /path/to/ltx2.safetensors --text-encoder-path /path/to/gemma \\
            --ref-image-col identity_image --ref-image-col style_image \\
            --ref-video-col motion_video
    """
