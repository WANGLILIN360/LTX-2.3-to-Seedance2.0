...
1. **Text-driven semantic routing (Seedance 2.0 approach)**: The model learns
   to understand "@Image1 for identity" from the **text encoder's output**, NOT
   from hand-crafted attention mask weights. All reference groups get full
   cross-attention (1.0) with target tokens. The model's cross-attention with
   the text context naturally routes each reference's influence.
...
3. **Attribute tags as training signals**: During training, `attribute_tags`
   serve as auxiliary supervision (e.g., loss weighting) rather than directly
   controlling attention masks. The model learns the routing end-to-end.

4. **Reference dropout**: During training, references are randomly dropped
   (default 10%) to enable classifier-free guidance style robustness.

5. **LoRA-only training**: The multi-reference strategy requires LoRA mode
   to avoid catastrophic forgetting of the base model's capabilities.

6. **Semantic parser for Chinese/mixed prompts**: The `SemanticPromptParser`
   uses Gemma 3 to extract structured bindings from Chinese prompts, but
   the **semantic understanding happens inside the diffusion model** via
   text cross-attention, not in the parser itself.
