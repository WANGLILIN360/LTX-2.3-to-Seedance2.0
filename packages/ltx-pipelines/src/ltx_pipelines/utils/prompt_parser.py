...
def _extract_attributes_for_mention(
    ...
    # Only look at text AFTER the mention (between this mention and the next).
    # We intentionally do NOT look before the mention to prevent attribute
    # bleed from previous mentions' context (e.g., "camera" leaking from
    # @Video1 into @Audio1's attributes).
    after_text = prompt[mention_end:context_end].lower()

    # Combined context (only after text)
    context = after_text
