...
class UnifiedMultiRefPipeline:
    ...
    def __call__(  # noqa: PLR0913
        ...
        # Build no-reference latent state for identity guidance (ID-LoRA style).
        # This state has the same noise as the conditioned state but excludes
        # multi-reference tokens, allowing the guider to extrapolate between
        # with-reference and without-reference predictions.
        noref_video_state = None
        if video_guider_params.identity_guidance_scale > 0 and stage_1_ref_conditionings:
            from ltx_core.components.patchifiers import VideoLatentPatchifier
            from ltx_core.tools import VideoLatentTools
            from ltx_core.types import VideoPixelShape, VideoLatentShape

            s1_pixel_shape = VideoPixelShape(
                batch=1, frames=num_frames, height=stage_1_height, width=stage_1_width, fps=frame_rate,
            )
            s1_v_shape = VideoLatentShape.from_pixel_shape(s1_pixel_shape)
            s1_tools = VideoLatentTools(VideoLatentPatchifier(patch_size=1), s1_v_shape, frame_rate)
            noref_video_state = create_noised_state(
                tools=s1_tools,
                conditionings=stage_1_image_conditionings,  # no multi-ref!
                noiser=noiser,
                dtype=self.dtype,
                device=self.device,
            )
    ...
    def _build_multi_ref_conditionings(
        ...
        audio_ref_paths: list[tuple[str, list[ReferenceAttribute], str]] = []
        ...
        for binding in parsed_prompt.bindings:
            ...
            if binding.ref_type == ReferenceModality.IMAGE:
                ...
                audio_ref_paths.append((ref_path, binding.attributes, binding.ref_id))
        ...
        for i, path in enumerate(audio_references):
            if (i + 1) not in bound_audio_indices:
                audio_ref_paths.append((path, [ReferenceAttribute.AUDIO_RHYTHM, ReferenceAttribute.AUDIO_MOOD], f"Audio{i + 1}"))
    ...
    def _encode_audio_references(
        ...
        ref_items: list[ReferenceItem] = []
        for ref_path, attributes, slot_id in audio_ref_paths:
